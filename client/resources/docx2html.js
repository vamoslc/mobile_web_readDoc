...
*/
	...
	if(typeof this.onload==="function") this.onload.call(this,html);	

	return this;
}
convertDocX.prototype.handleFileSelect = function(path, filename, typ){

	// evt.stopPropagation(); //stop 
	// evt.preventDefault(); //stop default action appear
	...
	if(typ == "docx"){
		...
		request.open('GET', path, true);//地址替换为自己dat文件的地址
		request.responseType = 'blob';
		var _obj = this
		request.onload = () => {
			console.log(toString.call(request.response));
			f = request.response;
		...
		.then((zip) => {
			...
			console.log(toString.call(images));
